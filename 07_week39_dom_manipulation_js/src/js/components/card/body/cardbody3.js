...
export class CardBody3 extends CardBody {
    ...
    draw() {
        ...
        this.element.innerHTML =
            '<p class="mb-2 fw-bold">Calculate Box Volume</p>' +
            '<form submit="" class="row mb-3 mb-lg-5">' +
            this.#getInput("Width (x)", "x") +
            this.#getInput("Height (y)", "y") +
            this.#getInput("Depth (z)", "z", "mb-0") +
            "</form>" +
            '<div id="result" class="mb-3 mb-lg-0">The volume is <strong><span></span> cm<sup>3</sup></strong>.</div>';
    ...
    #getInput(label, id, mb = "mb-2") {
        return (
            ...
            '<input type="number" min="0" step="0.1" class="form-control" id="' +
            id +
            '" required>' +
